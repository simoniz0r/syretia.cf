...
# Dependencies (only required for building from scratch): ar (binutils), fltk-dialog, xh
...
discord_dldeps() {
    ...
    latest_deb_url="$(xh -F "https://packages.ubuntu.com/$deb_release/amd64/$deb_name/download" | \
    grep '<li>*..*amd64\.deb' | \
    cut -f2 -d'"' | \
    head -n 1)"
    # exit if not found
    ...
    # download latest deb
    xh -F "$latest_deb_url" -o "$HOME"/.cache/"$version_lower"-appimage/debs/"$deb_name".deb || \
...
discord_setup() {
    ...
    # copy script to AppDir bin
    # CHANGE THIS TO DOWNLOAD FROM GIT REPO
    xh -F "https://bluepload.unstable.life/selif/discordappimage.sh" \
    ...
    xh -F "https://github.com/darealshinji/fltk-dialog/releases/download/continuous/fltk-dialog" \
    ...
    xh -F "https://discord.com/api/download$version_url?format=deb&platform=linux" \
    -o "$HOME"/.cache/"$version_lower"-appimage/debs/discord.deb || \
    discord_error "Error downloading $version_upper deb file" "1"
    # download xh deb (easy to use 'curl' replacement with no external deps)
    xh -F "https://github.com/ducaale/xh/releases/download/v0.13.0/xh_0.13.0_amd64.deb" \
    -o "$HOME"/.cache/"$version_lower"-appimage/debs/xh.deb || \
    discord_error "Error downloading xh deb file" "1"
    # extract discord and xh debs
    discord_extractdebs "discord.deb"
    discord_extractdebs "xh.deb"
    # edit build number for update testing
    # REMOVE ME
    # sed -i 's%0\.0\.16%0.0.15%' "$HOME"/.cache/"$version_lower"-appimage/AppDir/usr/share/"$version_lower"/resources/build_info.json
...
discord_buildappimage() {
    ...
    if [[ ! -w "$save_dir" ]]; then
        discord_error "Directory '$save_dir' is not writable" "1"
    fi
    # create AppDir
    ...
    xh -F "https://github.com/AppImage/AppImageKit/releases/download/continuous/appimagetool-x86_64.AppImage" \
    ...
    # make executable
    chmod +x "$save_dir"/"$version_lower"
    rm -rf "$HOME"/.cache/"$version_lower"-appimage
}
...
discord_update() {
    ...
    latest_ver="$(xh -h head "https://discord.com/api/download$version_url?format=deb&platform=linux" 2> /dev/null | \
    grep -im1 '^location:' | \
    cut -f6 -d'/')"
