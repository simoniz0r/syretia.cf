...
def lazypy [voice service text] {
    ...
    let audio_json = try { http post -m 10 -H [content-type application/x-www-form-urlencoded] https://lazypy.ro/tts/request_tts.php $body } catch { |e| return ($e.json | from json | select msg labels | wrap error) }
    # return response if success is not true
    if ($audio_json | get success) != true {
        return ($audio_json | wrap error)
    }
    let audio_url = try { $audio_json | get audio_url } catch { |e| return ($e.json | from json | select msg labels | wrap error) }
    # fetch and base64 encode result
    let base64 = try { http get -m 5 $audio_url | encode base64 } catch { |e| return ($e.json | from json | select msg labels | wrap error) }
...
def weilbyte [voice text] {
    ...
    let base64 = try { http post -m 6 -H [content-type application/json] https://tiktok-tts.weilbyte.dev/api/generate $body | encode base64 } catch { return (cursecode $voice $text) }
...
def uberduck [voice text] {
    ...
    let body = $text | wrap text | merge ($voice | wrap voice) | to json -r
    # make http post request
    let audio_json = try { http post -m 10 -H [content-type application/json] https://www.uberduck.ai/splash-tts $body } catch { |e| return ($e.json | from json | select msg labels | wrap error) }
    let audio_url = try { $audio_json | get response.path } catch { |e| return ($e.json | from json | select msg labels | wrap error) }
    # fetch and base64 encode result
    let base64 = try { http get -m 5 ($audio_url) | encode base64 } catch { |e| return ($e.json | from json | select msg labels | wrap error) }
    # wrap result for json otuput
...
def audiourl [voice text] {
    ...
    let base64 = try { http get -m 5 $enc_url | encode base64 } catch { |e| return ($e.json | from json | select msg labels | wrap error) }
