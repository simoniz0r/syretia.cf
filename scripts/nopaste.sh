...
def main [...body] {
    ...
    if $in_paste != "" {
        ...
        let decoded = try { $in_paste | base64 -dw0 | complete } catch { |e| return ($e.msg | wrap error | to json -r) }
        # return if exit not 0
        if ($decoded | get exit_code) != 0 { return ($decoded | wrap error | to json -r) }
        # try to decompress decoded input
        let decompressed = try { $decoded | get stdout | lzma -d | complete } catch { |e| return ($e.msg | wrap error | to json -r) }
        # return if exit not 0
        ...
        let lzma_base64 = try { $body_paste | lzma | base64 -w0 | complete } catch { |e| return ($e.msg | wrap error | to json -r) }
        # return if exit not 0
